...
import {
    X, Save, Ruler, Check, Trash2, AlertTriangle, RefreshCw
} from 'lucide-react';
...
export default function ModalTamano({
    isOpen, onClose, onSave, onDelete, tamanoAEditar
}: ModalTamanoProps) {
    ...
    return (
        <>
        <style>{`
            /* Mismo CSS que ModalCategoria para no duplicar en este código, se asume inyectado o global */
            ...
            {isOpen && (
                ...
                {step === 'confirm-delete' ? 'Esta acción no se puede deshacer' : step.startsWith('success') ? 'Operación completada' : esEdicion ? 'Modifica la descripción' : 'Registra un tamaño'}
                </span>
                </div>
                </div>
                ...
                {step === 'form' && (
                    ...
                    <div>
                    <label className="me-label"><Ruler size={13} /> Descripción</label>
                    <input type="text" className="me-input" placeholder="Ej. Chico, Mediano, Grande, 5cm..." value={descripcion} onChange={e => setDescripcion(e.target.value)} required disabled={loading} />
                    </div>
                    <div style={{ display:'flex', flexDirection:'column', gap:10, marginTop:4 }}>
                    <motion.button type="submit" className={`me-save-btn${esEdicion ? ' edit' : ''}`} disabled={loading || !descripcion} whileHover={!loading ? { scale:1.01 } : {}} whileTap={!loading ? { scale:0.97 } : {}}>
                    {loading ? <><span className="me-spinner"><RefreshCw size={16} /></span> Guardando...</> : <><Save size={16} /> {esEdicion ? 'Actualizar' : 'Guardar'} tamaño</>}
                    </motion.button>
                    {esEdicion && onDelete && (
