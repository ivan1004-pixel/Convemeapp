...
export const getCortes = async (skip = 0, take = 20, search: string = '') => {
    const query = `
    query GetCortesVendedor($skip: Int, $take: Int, $search: String) {
        cortesVendedor(skip: $skip, take: $take, search: $search) {
            id_corte
            fecha_corte
            dinero_esperado
