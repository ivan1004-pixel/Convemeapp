...
export default function AdminCorteDetail() {
  ...
  const handleSaveComprobante = async () => {
    ...
    try {
      ...
      const totalUnits =
      corte.detalles?.reduce(
        (acc: number, d: any) => acc + d.cantidad_vendida,
                             0,
      ) || 0;
  ...
  return (
    ...
    CORTE FOLIO #{corte.id_corte}
    </Text>
    <Text style={styles.dateLabel}>
    {formatDate(corte.fecha_corte).toUpperCase()}
    </Text>
    </View>
    ...
    {formatCurrency(corte.dinero_esperado)}
    </Text>
    </View>
    <View style={styles.row}>
    <Text style={styles.label}>TOTAL ENTREGADO:</Text>
    <Text style={styles.value}>
    {formatCurrency(corte.dinero_total_entregado)}
    </Text>
    </View>
    ...
    style={[
      styles.difValue,
      { color: getDiferenciaColor(corte.diferencia_corte) },
    ]}
    >
    {corte.diferencia_corte > 0.01 ? '+' : ''}
    ...
    {corte.observaciones ? (
      ...
      {corte.observaciones.toUpperCase()}
      </Text>
      </View>
    ) : null}
    ...
    {corte.detalles && corte.detalles.length > 0 && (
      ...
      {corte.detalles.map((det: any, index: number) => (
        ...
        {det.producto?.nombre?.toUpperCase() ||
    ...
    onPress={() =>
      router.push({
        pathname: '/(app)/cortes/create',
                  params: { id: String(corte.id_corte) },
      })
    }
    style={[styles.actionBtn, styles.outlineActionBtn]}
    />
