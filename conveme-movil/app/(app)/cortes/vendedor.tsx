...
function CorteCard({ item, onPress }: { item: Corte; onPress: () => void }) {
      ...
      <SafeAreaView style={styles.container} edges={['top']}>
        ...
        {loading && cortes.length === 0 ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <CorteCard item={item} onPress={() => router.push(`/cortes/${item.id_corte}`)} />
            )}
...
const styles = StyleSheet.create({
  ...
  addBtn: { width: 44, height: 44, borderRadius: 12, backgroundColor: Colors.primary, borderWidth: 2, borderColor: Colors.dark, alignItems: 'center', justifyContent: 'center', shadowColor: Colors.dark, shadowOffset: { width: 3, height: 3 }, shadowOpacity: 1 },
  listContent: { paddingHorizontal: Spacing.lg, paddingBottom: 100 },
  card: { backgroundColor: '#FFFFFF', borderRadius: BorderRadius.xl, padding: Spacing.lg, marginBottom: Spacing.md, borderWidth: 2, borderColor: Colors.dark, shadowColor: Colors.dark, shadowOffset: { width: 4, height: 4 }, shadowOpacity: 0.1 },
