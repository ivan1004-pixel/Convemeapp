...
export default function CorteCreateScreen() {
  ...
  useEffect(() => {
    if (isEditing && !isAdmin) {
      ...
      setTimeout(() => router.replace('/cortes'), 2000);
    }
  }, [isEditing, isAdmin]);
  ...
  const totalVendidos = form.detalles.reduce(
    (acc, d) => acc + d.cantidad_vendida,
                                             0,
  );
  const ventaBruta = form.detalles.reduce(
    (acc, d) => acc + d.cantidad_vendida * (d.precio || 0),
                                          0,
  );
  ...
  const validate = (): boolean => {
    ...
    const inventoryError = form.detalles.some(
      (d) =>
      d.cantidad_vendida +
      d.cantidad_devuelta +
      d.merma_reportada !==
    ...
    if (inventoryError) {
      showToast(
        'El inventario reportado no coincide con lo asignado',
        'error',
      );
      return false;
    }
  ...
  const handleSaveComprobante = async () => {
    ...
    try {
      const bruteTotal =
      lastCreatedCorte.detalles?.reduce(
        (acc, d) =>
        acc + d.cantidad_vendida * (d.producto?.precio_unitario || 0),
                                        0,
      ) || 0;
      const totalUnits =
      lastCreatedCorte.detalles?.reduce(
  ...
  const selectedAsignacion = asignaciones.find(
    (a) => a.id_asignacion === form.asignacion_id,
  );
  ...
  asignaciones.some(
    (a) =>
    a.vendedor?.id_vendedor === v.id_vendedor &&
    a.estado !== 'Finalizado',
  ),
  );

  const filteredVendedores = vendedoresConAsignacion.filter((v) =>
  v.nombre_completo.toLowerCase().includes(searchQuery.toLowerCase()),
  );
  const availableAsignaciones = asignaciones.filter(
    (a) =>
    a.vendedor?.id_vendedor === form.vendedor_id &&
    a.estado !== 'Finalizado',
  );
  ...
  return (
    ...
    <Pressable onPress={() => router.replace('/cortes')} style={styles.backBtn}>
    ...
    {isEditing ? 'Editar Corte' : 'Realizar Corte'}
    </Text>
    ...
    style={[
      styles.selector,
      errors.vendedor_id && styles.selectorError,
    ...
    style={[
      styles.selectorText,
      !selectedVendedor && styles.placeholderText,
    ]}
    >
    {selectedVendedor
      ? selectedVendedor.nombre_completo.toUpperCase()
      ...
      style={[
        styles.selector,
        errors.asignacion_id && styles.selectorError,
      ]}
      onPress={() => {
      ...
      style={[
        styles.selectorText,
        !selectedAsignacion && styles.placeholderText,
      ]}
      >
      {selectedAsignacion
        ? `ASIGNACIÓN #${selectedAsignacion.id_asignacion} (${formatDate(
        ...
        {form.detalles.length > 0 && (
          ...
          {form.detalles.map((det, index) => {
            ...
            return (
              ...
              <Text style={styles.asignedBadge}>
              ENTREGADO: {det.cantidad_asignada}
              </Text>
              </View>
              ...
              onChangeText={(v) =>
                updateDetalle(index, 'cantidad_vendida', v)
              }
              ...
              onChangeText={(v) =>
                updateDetalle(index, 'cantidad_devuelta', v)
              }
              keyboardType="numeric"
              inputStyle={styles.miniInputStyle}
              />
              ...
              onChangeText={(v) =>
                updateDetalle(index, 'merma_reportada', v)
              }
              keyboardType="numeric"
              inputStyle={styles.miniInputStyle}
              />
              ...
              {faltante !== 0 && (
                ...
                {faltante > 0
                  ? `Faltan ${faltante} piezas por reportar`
                  : `Sobran ${Math.abs(
                    faltante,
                  )} piezas reportadas`}
                  </Text>
        ...
        Venta Bruta ({totalVendidos} unids):
        </Text>
        <Text style={styles.calcValue}>
        {formatCurrency(ventaBruta)}
        </Text>
        </View>
        ...
        >
        - {formatCurrency(comisionVendedor)}
        </Text>
        </View>
        ...
        <Button
        title={
          isEditing ? 'ACTUALIZAR CORTE' : 'FINALIZAR Y CERRAR CUENTA'
        }
        onPress={handleSubmit}
        loading={submitting}
        size="lg"
        variant={
          Math.abs(diferencia) < 0.01 ? 'primary' : 'secondary'
        }
        ...
        >
        {lastCreatedCorte && (
          <CorteTicket corte={lastCreatedCorte} />
        )}
        </ScrollView>
        <View style={{ gap: 10 }}>
        <Button
        ...
        name="shield-lock"
        size={48}
        color={diferencia < 0 ? Colors.error : Colors.success}
        />
        <Text
        style={[
          styles.authTitle,
  ...
  . Ingresa tu contraseña para autorizar el cierre.
  </Text>
  ...
  {isAuthorizing ? (
    ...
    <Text style={styles.authConfirmText}>AUTORIZAR</Text>
  )}
  </TouchableOpacity>
  </View>
  </View>
  ...
  ListEmptyComponent={
    ...
    No hay vendedores con asignaciones pendientes
    </Text>
  }
  />
  </View>
  ...
  <TouchableOpacity
  onPress={() => setShowAsignacionModal(false)}
  >
  <MaterialCommunityIcons
  name="close-thick"
  size={28}
  ...
  renderItem={({ item }) => (
    ...
    <Text style={styles.modalListItemSub}>
    Fecha: {formatDate(item.fecha_asignacion)}
    </Text>
    </View>
    </TouchableOpacity>
  )}
  ListEmptyComponent={
    <Text style={styles.emptyText}>
    No hay asignaciones pendientes para este vendedor
    </Text>
  }
  />
  </View>
