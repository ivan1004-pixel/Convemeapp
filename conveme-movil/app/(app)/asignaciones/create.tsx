...
export default function AsignacionCreateScreen() {
  ...
  const isReadOnly = existing?.estado === 'Finalizado' || existing?.estado === 'Entregado';
  ...
  const validate = (): boolean => {
    ...
    if (invalidQty) newErrors.detalles = 'TODAS LAS CANTIDADES DEBEN SER MAYORES A 0';
  ...
  const handleSubmit = async () => {
    if (isReadOnly) return;
    if (!validate()) {
      showToast(errors.vendedor_id || errors.detalles || 'COMPLETA LOS CAMPOS REQUERIDOS', 'warning');
      return;
    }
    setSubmitting(true);
    try {
      ...
      setTimeout(() => router.replace('/asignaciones'), 1500);
    } catch (err) {
  ...
  return (
    ...
    <Pressable onPress={() => router.replace('/asignaciones')} style={styles.backBtn}>
    ...
    {isReadOnly && (
      ...
      <Text style={styles.readonlyText}>ESTA ASIGNACIÓN ESTÁ {existing?.estado?.toUpperCase()} Y NO SE PUEDE EDITAR</Text>
      </View>
    )}
    ...
    {errors.vendedor_id && <Text style={styles.errorTextItem}>{errors.vendedor_id}</Text>}
...
const styles = StyleSheet.create({
  ...
  placeholderText: { color: 'rgba(0,0,0,0.3)' },
                                 infoBox: { marginTop: 12, padding: 10, backgroundColor: '#F3F4F6', borderRadius: 8, borderWidth: 1, borderColor: 'rgba(0,0,0,0.05)' },
                                 infoLabel: { fontSize: 10, fontWeight: '800', color: 'rgba(0,0,0,0.5)', textTransform: 'uppercase', marginBottom: 2 },
                                 ...
                                 errorTextItem: { color: Colors.error, fontSize: 10, fontWeight: '800', marginTop: 4, marginLeft: 4 },
